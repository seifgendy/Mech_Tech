...
void SPI_transieve(u8 TxData, u8* RxData)
{
	if(RxData!=NULL)
	{
		//Check on Colision Flag (trying to write data before completing the sending)
...
void SPI_transmitAsynchronous(u8 TxData, void(*ptrToFun)(u8))
{
	if(ptrToFun!=NULL)
	{
		// SPI Interrupt Enable LOCAL INTERRUPT
		SET_BIT(SPI->SPCR,SPIE);
		
		private_pCallBackSPI = ptrToFun;//should prepare the func that will do the action after the interrupt
